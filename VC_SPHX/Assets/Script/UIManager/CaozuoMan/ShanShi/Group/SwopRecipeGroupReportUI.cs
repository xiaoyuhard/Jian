...
/// 群体膳食食谱交换界面 出报告中
/// </summary>
public class SwopRecipeGroupReportUI : MonoSingletonBase<SwopRecipeGroupReportUI>
{
    ...
    {
        ServerCon.Instance.LoadRecipe("/cookbook/getGroupRecipeFood", $"?recipeId={recipeItem.id}", "SwopRecipeDay");
    ...
    private void ConfirmFoodItem()
    {
        ...
        if (IsInputList() && (countRecipeOld == countRecipeNew))
        {
            ...
            MealGroupReportUI.Instance.SwopRecipeGroup(oldRecipe, newRecipe);
            return;
        }
