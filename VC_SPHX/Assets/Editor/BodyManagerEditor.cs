    ...
    public override void OnInspectorGUI()
    {
        ...
        foreach (var kvp in dm.itemBoneDic)
        {
            ...
            foreach (var item in kvp.Value)
            {
                ...
                EditorGUILayout.BeginVertical("Box");
                EditorGUILayout.LabelField($"编号: {item.Level1}");
                EditorGUILayout.LabelField($"名称: {item.Level2}");
                EditorGUILayout.LabelField($"单位: {item.Level3}");
                EditorGUILayout.LabelField($"单位: {item.Level4}");
        ...
        foreach (var kvp in dm.itemEndocrDic)
        {
            ...
            foreach (var item in kvp.Value)
            {
                ...
                EditorGUILayout.BeginVertical("Box");
                EditorGUILayout.LabelField($"编号: {item.Female}");
                EditorGUILayout.LabelField($"名称: {item.Male}");
                EditorGUILayout.EndVertical();
                count++;
...
{
    [MenuItem("Tools/Body/快速访问窗口")]
    static void ShowDataWindow()
    {
        BodyBrowserWindow.ShowWindow();
    }

    [MenuItem("Tools/Body/打开Excel目录")]
...
{
    [MenuItem("Tools/Body/数据浏览窗口")]
    public static void ShowWindow()
    {
        GetWindow<BodyBrowserWindow>("数据浏览器");
    }
    ...
    void OnGUI()
    {
        if (BodyManager.Instance == null)
        {
            EditorGUILayout.HelpBox("BodyManager实例不存在！", MessageType.Error);
        ...
        //EditorGUILayout.LabelField($"已加载表数量: {BodyManager.Instance.itemDictionary.Count}");
